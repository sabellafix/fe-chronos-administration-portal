...
@Component({
  ...
  styleUrl: './companies-update.component.scss'
})
export class CompaniesUpdateComponent implements OnInit {
  titleComponent: string = "Actualizar empresa";
  ...
  put() {
    ...
    if (this.form.valid) {
      ...
      this.companyService.put(put, parseInt(this.id)).subscribe({
        next: (data: any) => {
          this.charge = false;
          let company = <Company>data;
          this.snackBar.open('Empresa actualizada correctamente.', 'Cerrar', {duration: 4000});
